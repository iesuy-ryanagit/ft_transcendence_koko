const apiBase = 'https://localhost:8000/api/';
...
function navigateTo(page, addHistory = true) {
    ...
    // 表示するページを決定
    if (page === 'login-base') {
        document.getElementById('login-base').classList.remove('d-none');
    } 
    else if (page === 'login') {
...
// 画面切り替え関数
function showLoginBase(addHistory = true) {
    navigateTo('login-base', addHistory);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page = location.hash.replace('#', '') || 'login-base';
    navigateTo(token ? page : 'login-base', false);
});
